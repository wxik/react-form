...
export class CCListWrapper extends Component<ICCList, ICCListState> {
  ...
  initState() {
    ...
    if (context && context.initialValue) {
      if (!isBlank(formName)) {
        initialValue = get(context.initialValue, formName, initialValue);
      } else if (isArray(context.initialValue)) {
    ...
    if (isArray(initialValue) && values.length) {
  ...
  addItem(item?: any, insertIndex?: number) {
    ...
    if (!isEmpty(insertIndex) && insertIndex < data.length) {
  ...
  removeItem(index: number | number[]) {
    ...
    let nowIndex = (isArray(index) ? index : [index]).filter((it) => it < size);
    if (nowIndex.length) {
  ...
  removeOutData(size: number) {
    ...
    Object.keys(data).forEach((fi) => {
      if (isBlank(formName)) {
        ...
        fi.indexOf(String(formName)) === 0 &&
        inForms.findIndex((da) => fi.indexOf(da) !== -1) === -1
      ) {
        formInstance.deleteField(fi, { isChange: false });
      }
    });
  ...
  componentDidUpdate(
    prevProps: Readonly<ICCList>,
    prevState: Readonly<ICCListState>,
    snapshot?: any,
  ) {
